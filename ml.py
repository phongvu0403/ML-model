...
for name, model in models:
    ...
    cv_results = cross_val_score(model, X_visual, y, cv=kfold, scoring="neg_mean_squared_error")
    model.fit(X_visual,y)
    #val = mean_squared_error(model.predict(X_test), y_test)
    train = mean_squared_error(model.predict(X_visual), y)
...
FinalModel.fit(X_visual,y)
y_pred=FinalModel.predict(X_visual).round()
kfold = KFold(n_splits=10, random_state=7)
cv_results = cross_val_score(FinalModel, X_visual, y, cv=kfold, scoring="neg_mean_squared_error")
